...
async function generateHTML(SourcePagesPath, destination) {
  ...
  templateFolders.forEach(async function (folderName) {
    // file.pug { + [option.json] + ... post.markdown + indexes.pug(posts) }
  ...
  pugNormalFiles.forEach(async function (fileName) {
    // file.pug { + option.json }
    handlePugFile(path.join(SourcePagesPath, fileName), destination)
  })
  // ${SourcePagesPath} => ${destination} sub folders : ${subFolders} | template folders : ${templateFolders} | ${pugNormalFiles}
  return `SUCCESS: ${SourcePagesPath} => ${destination} HTMLs generated!`
}
